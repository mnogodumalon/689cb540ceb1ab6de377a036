...
import { IconAlertCircle, IconPlus, IconPencil, IconTrash, IconUsers, IconClock, IconMapPin, IconFileText, IconFiretruck, IconShieldCheck, IconCalendar } from '@tabler/icons-react';
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          <Button
            ...
            className="gap-1.5"
          >
            <IconFiretruck size={15} className="shrink-0" stroke={1.5} />
          ...
          <Button
            ...
            className="gap-1.5"
          >
            <IconPlus size={15} className="shrink-0" stroke={1.5} />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard
          ...
          icon={<IconUsers size={18} className="text-muted-foreground" stroke={1.5} />}
        />
        <StatCard
          title="Aktive Dienste"
          value={String(activeShifts.length)}
          description="Jetzt im Einsatz"
          icon={<IconShieldCheck size={18} className="text-green-600" stroke={1.5} />}
        />
        <StatCard
          title="Geplante Dienste"
          value={String(upcomingShifts.length)}
          description="Bevorstehend"
          icon={<IconCalendar size={18} className="text-blue-600" stroke={1.5} />}
        />
        <StatCard
          title="Einsatzprotokolle"
          value={String(einsatzprotokoll.length)}
          description="Gesamt"
          icon={<IconFileText size={18} className="text-muted-foreground" stroke={1.5} />}
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2 space-y-3">
          ...
          {sortedShifts.length === 0 && (
            <div className="flex flex-col items-center justify-center py-12 text-center rounded-2xl border border-dashed border-border">
              <IconCalendar size={36} className="text-muted-foreground mb-3" stroke={1.5} />
              ...
              <Button size="sm" className="mt-4" onClick={() => { setEditShift(null); setShiftDialogOpen(true); }}>
                <IconPlus size={14} className="mr-1.5" stroke={1.5} /> Dienst anlegen
              </Button>
            </div>
          )}
        </div>
        ...
        <div className="lg:col-span-3 space-y-3">
          <div className="flex items-center justify-between">
            ...
            >
              <IconPlus size={13} className="shrink-0" stroke={1.5} />
              Einsatz
            </Button>
          </div>
          ...
          <div className="space-y-2 overflow-x-auto">
            ...
            {(selectedShiftId ? incidentsForShift : recentIncidents).length === 0 && (
              <div className="flex flex-col items-center justify-center py-12 text-center rounded-2xl border border-dashed border-border">
                <IconFiretruck size={36} className="text-muted-foreground mb-3" stroke={1.5} />
                ...
                >
                  <IconPlus size={14} className="mr-1.5" stroke={1.5} /> Einsatz melden
                </Button>
              </div>
            )}
          </div>
    ...
    >
      <div className="flex items-start justify-between gap-2 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1.5 text-xs text-muted-foreground">
            {start && (
              <span className="flex items-center gap-1">
                <IconClock size={11} className="shrink-0" stroke={1.5} />
                {formatDate(start)}
                {end && <> – {formatDate(end)}</>}
              </span>
            )}
            {shift.mitgliederName && (
              <span className="flex items-center gap-1">
                <IconUsers size={11} className="shrink-0" stroke={1.5} />
                <span className="truncate max-w-[120px]">{shift.mitgliederName}</span>
              </span>
            )}
            {incidentCount > 0 && (
              <span className="flex items-center gap-1 text-orange-600">
                <IconFiretruck size={11} className="shrink-0" stroke={1.5} />
        ...
        <div className="flex gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" onClick={e => e.stopPropagation()}>
          ...
          >
            <IconFiretruck size={13} stroke={1.5} />
          </button>
          <button
            title="Bearbeiten"
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap min-w-0">
            <div className="w-7 h-7 rounded-xl bg-orange-500/10 flex items-center justify-center shrink-0">
              <IconFiretruck size={14} className="text-orange-600" stroke={1.5} />
            </div>
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1.5 text-xs text-muted-foreground ml-9">
            ...
            {incident.bereitschaftsbezugName && (
              <span className="flex items-center gap-1 truncate max-w-[150px]">
                <IconMapPin size={11} className="shrink-0" stroke={1.5} />
                {incident.bereitschaftsbezugName}
              </span>
            )}
