...
interface PersonalverwaltungDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Personalverwaltung['fields']) => Promise<void>;
  defaultValues?: Personalverwaltung['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function PersonalverwaltungDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = false, enablePhotoLocation = true }: PersonalverwaltungDialogProps) {
  const [fields, setFields] = useState<Partial<Personalverwaltung['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'personalverwaltung');
      await onSubmit(clean as Personalverwaltung['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      const schema = `{\n  "vorname": string | null, // Vorname\n  "telefon": string | null, // Telefonnummer\n  "email": string | null, // E-Mail-Adresse\n  "qualifikation": LookupValue[] | null, // Qualifikationen (select one or more keys: "atemschutz" | "maschinist" | "gruppenfuehrer" | "sanitaeter" | "funker") mapping: atemschutz=Atemschutzgeräteträger, maschinist=Maschinist, gruppenfuehrer=Gruppenführer, sanitaeter=Sanitäter, funker=Funker\n  "nachname": string | null, // Nachname\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Personalverwaltung['fields']>;
      });
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Personalverwaltung bearbeiten' : 'Personalverwaltung hinzufügen';
